...
void main() {
  ...
  test('renders sections and rows in the order given', () {
    ...
    expect(
      text.indexOf('- Files: 120'),
      lessThan(text.indexOf('- Conflicts: 0')),
    );
  });
  ...
  test(
    'a log notice explains an empty log instead of leaving it ambiguous',
    () {
      final text = _report(
        logNotice: 'The log file could not be read.',
      ).render();
  ...
  group('redaction', () {
    ...
    test('strips passphrases and storage keys', () {
      expect(
        redactSecrets('passphrase=hunter2000'),
        isNot(contains('hunter2000')),
      );
      expect(
    ...
    test('keeps the hashes and ids that make a report debuggable', () {
      const line =
          'blobRef=e3b0c44298fc1c149afbf4c8996fb924'
