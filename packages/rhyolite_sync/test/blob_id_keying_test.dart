...
void main() {
  group('VaultCipher.deriveBlobIdKey', () {
    test('deterministic, 32 bytes, key-separated from the AES key', () async {
      final c = await VaultCipher.derive(
    ...
    test('differs across passphrase and across vault', () async {
      final a = (await VaultCipher.derive(
        'pass-A',
        'vault-1',
      )).deriveBlobIdKey();
      final b = (await VaultCipher.derive(
        'pass-B',
        'vault-1',
      )).deriveBlobIdKey();
      final c = (await VaultCipher.derive(
  ...
  group('keyed content-addressing', () {
    test('chunker with a keyed hasher yields HMAC ids, not sha256', () async {
      ...
      final keyedHash = (await keyed(data)).manifest.chunks.single.hash;
      final plainHash = (await plain(data)).manifest.chunks.single.hash;
    ...
    test(
      'ChunkedBlobIO: keyed ids differ from unkeyed and still round-trip',
      () async {
    ...
    test(
      'within-vault dedup still works with a key (same content → same id)',
      () async {
