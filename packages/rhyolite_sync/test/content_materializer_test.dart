...
void main() {
  test(
    'a text path projects the Fugue blob to plain text (not \\0fg1 bytes)',
    () async {
      final blob = FugueStore.encodeBlob(
        FugueTextSync.seedFromText('hello world'),
      );
      // The raw blob carries the magic header — must NOT reach disk / the diff.
      expect(blob.sublist(0, 4), [0x00, 0x66, 0x67, 0x31]);

      final out = materializeFileContent(blob, 'notes/a.md', isTextPath: true);
  ...
  test('a binary path passes the blob through unchanged', () {
    ...
    expect(
      materializeFileContent(bytes, 'img/pic.png', isTextPath: false),
      same(bytes),
    );
  });
  ...
  test('a genuine pre-Fugue plain-text blob on a text path passes through', () {
    ...
    expect(
      utf8.decode(materializeFileContent(bytes, 'a.txt', isTextPath: true)!),
      'legacy plain',
    );
  });
}
