...
void main() {
  group('WebDAV listing', () {
    test(
      'yields the object names directly under the vault directory',
      () async {
    ...
    test(
      '404 on the vault directory is an empty bucket, not a failure',
      () async {
        final ids = await _dav(
  ...
  group('S3 listing', () {
    ...
    test(
      'query parameters are sorted, because SigV4 signs them verbatim',
      () async {
        final queries = <String>[];
        await _s3(
          MockClient((request) async {
    ...
    test(
      'an error on the first page answers null, not a clean bucket',
      () async {
        final ids = await _s3(
          MockClient((_) async => http.Response('denied', 403)),
        ).listBlobIds();
    ...
    test('an error on a later page keeps what was already listed', () async {
      ...
      expect(
        ...
        ['one'],
        reason:
            'a short listing costs a later sweep, never data — the '
